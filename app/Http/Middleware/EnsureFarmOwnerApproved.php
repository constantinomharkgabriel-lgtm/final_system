...
class EnsureFarmOwnerApproved
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        // Check specific allowed route names
        if (in_array($routeName, $this->allowedRouteNames, true)) {
            return $next($request);
        }

        // Check if route matches allowed patterns (drivers.*, deliveries.*)
        if (str_starts_with($routeName, 'drivers.') || str_starts_with($routeName, 'deliveries.')) {
            return $next($request);
        }
