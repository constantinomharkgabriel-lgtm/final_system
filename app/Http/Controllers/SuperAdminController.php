...
class SuperAdminController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            $recent_farm_owners = FarmOwner::with('user:id,name,email')
                ->select('id', 'user_id', 'farm_name', 'city', 'province', 'permit_status', 'created_at')
                ->latest('created_at')
                ->limit(10)
                ->get();
    ...
    public function approve_farm_owner($id)
    {
        ...
        // Copy documents from ClientRequest to FarmOwner if they exist
        $clientRequest = ClientRequest::where('email', $farm_owner->user?->email)->first();
        if ($clientRequest) {
            $farm_owner->update([
                'valid_id_path' => $clientRequest->valid_id_path,
                'business_permit_path' => $clientRequest->business_permit_path,
                'barangay_clearance_path' => $clientRequest->barangay_clearance_path,
                'mayor_bir_registration_path' => $clientRequest->mayor_bir_registration_path,
                'ecc_certificate_path' => $clientRequest->ecc_certificate_path,
                'bai_registration_path' => $clientRequest->bai_registration_path,
                'locational_clearance_path' => $clientRequest->locational_clearance_path,
