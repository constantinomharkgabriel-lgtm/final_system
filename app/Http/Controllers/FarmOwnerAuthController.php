...
class FarmOwnerAuthController extends Controller
{
    ...
    {
        // Check if email belongs to a rejected farm owner (allows retry)
        $existingUser = User::where('email', $request->email)->where('role', 'farm_owner')->first();
        $isRejectedRetry = false;
        
        if ($existingUser) {
            $farmOwner = FarmOwner::where('user_id', $existingUser->id)->first();
            // If farm owner exists and is rejected, allow re-registration
            if ($farmOwner && $farmOwner->permit_status === 'rejected') {
                $isRejectedRetry = true;
            }
        }

        // Build validation rules with conditional email/phone uniqueness
        $rules = [
            'name' => 'required|string|max:255',
            'email' => ['required', 'email:rfc,dns'],
            'password' => 'required|string|min:8|confirmed',
            'phone' => ['nullable', new PhilippinePhoneNumber()],
        ...
        // Only enforce uniqueness if not a rejected retry
        if (!$isRejectedRetry) {
            ...
            // For rejected retries, allow new farm_name and business_registration if different
            $existingFarmOwner = FarmOwner::where('user_id', $existingUser->id)->first();
        ...
        $phone = $validated['phone'] ? PhilippinePhoneNumber::normalize($validated['phone']) : null;
        
        if ($isRejectedRetry) {
            // Update existing user (rejected farm owner retry)
            $existingUser->update([
                'name' => $validated['name'],
                'phone' => $phone,
            ]);

            // Update or create farm owner profile
            $existingFarmOwner = FarmOwner::where('user_id', $existingUser->id)->first();
            $existingFarmOwner->update([
                ...
                'permit_status' => 'pending', // Reset to pending for re-review
            ]);

            Log::info('Farm owner registration retry', [
            ...
            return redirect()->route('farmowner.pending')->with('success', 'Farm registration resubmitted successfully. Awaiting admin verification.');
        } else {
            // Create new user and farm owner
            $user = User::create([
                'name' => $validated['name'],
    ...
    public function logout(Request $request)
    {
        ...
        return redirect()->away('https://poultryandsupplies.com');
    }
}
