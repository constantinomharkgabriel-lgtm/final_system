...
class PayrollController extends Controller
{
    ...
    /**
     * Calculate working days in a given month, excluding Sundays.
     * Philippines standard: typically 20-23 working days per month.
     */
    private function calculateWorkingDaysInMonth($year, $month): int
    {
        $workingDays = 0;
    ...
    public function index(Request $request)
    {
        ...
        $baseQuery = Payroll::byFarmOwner($farmOwner->id)
            ->with('employee:id,first_name,last_name,position')
            ->select('id', 'farm_owner_id', 'employee_id', 'payroll_period', 'period_start', 'period_end', 'basic_pay', 'net_pay', 'status', 'pay_date', 'workflow_status', 'created_at');
        ...
        if ($request->filled('month')) {
            ...
            $baseQuery->where('period_start', '>=', $monthStart)
                ->where('period_start', '<', $nextMonthStart);
        }

        // Paginate with fewer records per page for faster loading
        $payrolls = $baseQuery->latest('period_start')->paginate(10);

        // Cache stats for 5 minutes to reduce queries
        $cacheKey = 'payroll_stats_' . $farmOwner->id;
        $stats = Cache::remember($cacheKey, 300, function () use ($farmOwner) {
            return [
                'total' => Payroll::byFarmOwner($farmOwner->id)->count(),
                'pending' => Payroll::byFarmOwner($farmOwner->id)->where('workflow_status', 'pending_finance')->count(),
                'approved' => Payroll::byFarmOwner($farmOwner->id)->where('status', 'approved')->count(),
                'paid' => Payroll::byFarmOwner($farmOwner->id)->where('status', 'paid')->count(),
            ];
        });

        // Return view based on user role
        if (Auth::user()->isFarmOwner()) {
            $view = 'farmowner.payroll.index';
        } elseif (Auth::user()->isHR()) {
            $view = 'hr.payroll.index';
        } else {
            $view = 'department.payroll.index';
        }

        return view($view, compact('payrolls', 'stats'));
    }
    ...
    public function store(Request $request)
    {
        ...
        // Calculate delivery commission if employee is a driver
        $deliveryCommission = 0;
        $deliveryCount = 0;
        $employee = Employee::with(['driver', 'roles'])->find((int) $validated['employee_id']);
        
        if ($employee && $employee->hasRole('driver') && $employee->driver) {
            // Count completed deliveries in the payroll period
            $deliveryCount = $employee->driver->deliveries()
                ->where('status', 'completed')
                ->whereBetween('updated_at', [
                    Carbon::parse((string) $validated['period_start'])->startOfDay(),
                    Carbon::parse((string) $validated['period_end'])->endOfDay()
                ])
                ->count();
            
            // Calculate commission based on delivery fee * count
            if ($deliveryCount > 0) {
                $deliveryCommission = $deliveryCount * (float) ($employee->driver->delivery_fee ?? 0);
            }
        }

        $validated['delivery_count'] = $deliveryCount;
        $validated['delivery_commission'] = round($deliveryCommission, 2);

        // Calculate totals including delivery commission
        $grossPay = ($validated['basic_pay'] ?? 0) 
            + ($validated['overtime_pay'] ?? 0) 
            + ($validated['holiday_pay'] ?? 0) 
            + ($validated['delivery_commission'] ?? 0)
        ...
        // Invalidate cache
        Cache::forget('payroll_stats_' . $farmOwner->id);
    ...
    public function show(Payroll $payroll)
    {
        ...
        $payroll->load([
            'employee:id,first_name,last_name,position,performance_rating',
            'processedBy:id,name',
            'financeApprovedBy:id,name',
            'ownerApprovedBy:id,name',
            'payslipReleasedBy:id,name',
            'disbursementPreparedBy:id,name',
            'disbursedBy:id,name'
        ]);

        // Optimize edit requests query
        $editRequests = $payroll->editRequests()
            ->select('id', 'payroll_id', 'requested_by', 'reviewed_by', 'reason', 'status', 'requested_changes', 'created_at', 'reviewed_at')
            ->with([
        ...
        if (Auth::user()?->isFinance()) {
            return view('department.payroll.show', compact('payroll', 'editRequests'));
        } elseif (Auth::user()?->isHR()) {
            return view('hr.payroll.show', compact('payroll', 'editRequests'));
        }
    ...
    public function destroy(Payroll $payroll)
    {
        ...
        // Only HR can delete, and only before Finance approval
        abort_unless(Auth::user()?->isHR(), 403, 'Only HR can delete payroll.');
        abort_if(($payroll->workflow_status ?? 'draft') !== 'pending_finance', 403, 'Payroll can only be deleted before Finance approval.');
    ...
    public function executeDisbursement(Request $request, Payroll $payroll)
    {
        ...
        // For bank transfer and GCash, redirect to PayMongo payment
        if (in_array((string) $payroll->payment_method, ['bank_transfer', 'gcash'], true)) {
            return $this->initiatePayMongoPayment($payroll, Auth::id(), $farmOwner->id);
        }

        // For cash and check, validate that reference is provided
        if (in_array((string) $payroll->payment_method, ['check'], true)
            && blank($validated['disbursement_reference'] ?? null)) {
            return back()->with('error', 'Reference number is required for check disbursements.');
        }

        // For cash and check, mark as paid directly
        $payroll->update([
            'status' => 'paid',
    ...
    /**
     * Initiate PayMongo payment for bank transfer or GCash
     */
    private function initiatePayMongoPayment(Payroll $payroll, int $userId, int $farmOwnerId)
    {
        $paymongoService = app('Illuminate\Container\Container')->make(\App\Services\PayMongoService::class);
        
        $successUrl = route('payroll.paymongo-success', ['payroll' => $payroll->id]);
        $cancelUrl = route('payroll.show', ['payroll' => $payroll->id]);

        // Determine payment method for PayMongo
        $paymentMethods = ($payroll->payment_method === 'bank_transfer') 
            ? ['bank_transfer'] 
            : ['gcash'];

        $checkoutData = [
            'amount' => (int) ($payroll->net_pay * 100), // Convert to centavos
            'description' => 'Payroll Disbursement: ' . $payroll->payroll_period,
            'plan_name' => 'Payroll Payment',
        ...
        if (!$session) {
            return back()->with('error', 'Failed to create PayMongo checkout session. Please try again.');
        }

        // Store session ID for tracking
        $payroll->update([
            'paymongo_session_id' => $session['id'] ?? null,
            'payment_details' => [
                'session_created_at' => now()->toIso8601String(),
                'payment_method' => $payroll->payment_method,
                'amount' => $payroll->net_pay,
            ],
        ]);
        ...
        if (!$checkoutUrl) {
            return back()->with('error', 'PayMongo checkout URL could not be generated.');
        }
     ...
     */
    public function handlePayMongoSuccess(Request $request, Payroll $payroll)
    {
        $farmOwner = $this->getFarmOwner();
        abort_if($payroll->farm_owner_id !== $farmOwner->id, 403);

        // Verify that payment was completed via PayMongo webhook (in production)
        // For now, mark as paid with PayMongo reference
        ...
        $payroll->update([
            ...
            'disbursed_by' => Auth::id(),
            'disbursed_at' => now(),
            'disbursement_reference' => $payroll->paymongo_session_id,
        ]);
        ...
        Expense::updateOrCreate(
            ...
            [
                ...
                'status' => 'approved',
                'is_auto_generated' => true,
                'notes' => 'Paid via PayMongo ' . ucfirst(str_replace('_', ' ', $payroll->payment_method)) . '.',
            ]
        );
        ...
        return redirect()->route('payroll.show', $payroll)->with('success', 'Payroll disbursement completed via ' . ucfirst(str_replace('_', ' ', $payroll->payment_method)) . '.');
    }
    ...
    public function generateBatch(Request $request)
    {
        ...
        foreach ($employees as $employee) {
            ...
            // Calculate pay - Use daily_rate or monthly_salary ÷ 22 (Philippines DTI/BIR standard)
            $dailyRate = (float) ($employee->daily_rate ?: ((float) $employee->monthly_salary / 22));
