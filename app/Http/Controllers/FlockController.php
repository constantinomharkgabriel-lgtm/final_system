...
class FlockController extends Controller
{
    ...
    public function show(Flock $flock)
    {
        ...
        // Get accurate inventory total from egg_inventory (not from daily records sum)
        $eggInventoryTotal = \App\Models\EggInventory::where('farm_owner_id', $farmOwner->id)
        ...
        // Use inventory total instead of daily records sum for accuracy
        if ($recordStats) {
            $recordStats->total_eggs = $eggInventoryTotal;
        }
    ...
    {
        // Increase timeout for egg grading operations
        set_time_limit(300);
        ...
        $validated = $request->validate([
            ...
            'water_consumed_liters' => 'required|numeric|min:0',
        ...
        $existingRecord = FlockRecord::where('flock_id', $flock->id)
            ->where('record_date', $validated['record_date'])
            ->first();

        // Calculate the difference in eggs if updating
        $eggsDifference = $validated['eggs_collected'] ?? 0;
        $isUpdatingRecord = false;
        
        if ($existingRecord && isset($validated['eggs_collected'])) {
            $eggsDifference = ($validated['eggs_collected'] - ($existingRecord->eggs_collected ?? 0));
            $isUpdatingRecord = true;
        }

        // Update or create the record
        $flockRecord = FlockRecord::updateOrCreate(
        ...
        // Auto-grade eggs if eggs were collected
        if (($validated['eggs_collected'] ?? 0) > 0) {
            try {
                $eggService = new EggGradingService();
                $eggService->gradeAndCreateInventory(
                    $flock,
                    $validated['eggs_collected'] ?? 0,  // TOTAL eggs (not difference)
                    $validated['eggs_broken'] ?? 0,
                    $validated['record_date'],
                    $isUpdatingRecord,  // True if this is an update
                    $existingRecord ? ($existingRecord->eggs_collected ?? 0) : 0  // Previous value (if updating)
                );
            } catch (\Exception $e) {
                // Log error but don't fail the record save
                \Illuminate\Support\Facades\Log::warning('Egg grading failed', [
                    'flock_id' => $flock->id,
                    'error' => $e->getMessage(),
                ]);
            }
