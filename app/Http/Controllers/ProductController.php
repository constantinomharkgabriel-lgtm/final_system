...
class ProductController extends Controller
{
    ...
    public function create()
    {
        ...
        $eggInventory = EggInventory::where('farm_owner_id', $farm_owner->id)
            ->where('quantity_available', '>', 0)
            ->get()
            ->map(function($item) {
                return [
                    'id' => $item->id,
                    'type' => 'egg',
                    'label' => "Eggs: Grade {$item->grade} ({$item->size}), Quantity: {$item->quantity_available}",
                    'quantity' => $item->quantity_available,
                ];
            });

        $livestockInventory = LivestockInventory::where('farm_owner_id', $farm_owner->id)
            ...
            ->map(function($item) {
                return [
                    'id' => $item->id,
                    'type' => 'livestock',
                    'label' => "Livestock: " . ucfirst($item->livestock_type) . ", Quantity: {$item->quantity_available_for_sale}",
                    'quantity' => $item->quantity_available_for_sale,
                ];
            });

        // Check if coming from inventory page (pre-select inventory)
        $preSelectedInventoryType = request('inventory_type');
        $preSelectedInventoryId = request('inventory_id');
        $preSelectedItem = null;

        if ($preSelectedInventoryType && $preSelectedInventoryId) {
            // Only show the pre-selected inventory item
            if ($preSelectedInventoryType === 'egg') {
                $item = EggInventory::find($preSelectedInventoryId);
                if ($item && $item->farm_owner_id === $farm_owner->id) {
                    $preSelectedItem = [
                        'id' => $item->id,
                        'type' => 'egg',
                        'label' => "Eggs: Grade {$item->grade} ({$item->size}), Quantity: {$item->quantity_available}",
                    ...
                    $availableInventory = collect([$preSelectedItem]);
                }
            } elseif ($preSelectedInventoryType === 'livestock') {
                $item = LivestockInventory::find($preSelectedInventoryId);
                if ($item && $item->farm_owner_id === $farm_owner->id) {
                    $preSelectedItem = [
                        'id' => $item->id,
                        'type' => 'livestock',
            ...
            // Show all inventory only if not coming from inventory page
            $availableInventory = $eggInventory->concat($livestockInventory);
        }

        return view('farmowner.products.create', compact('availableInventory', 'preSelectedItem'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'link_to_inventory' => 'nullable|boolean',
            'inventory_type' => 'nullable|in:egg,livestock',
        ...
        $eggInventoryId = null;
        $livestockInventoryId = null;

        if ($validated['link_to_inventory'] ?? false) {
            $inventoryType = $validated['inventory_type'] ?? null;
            $inventoryId = $validated['inventory_id'] ?? null;

            if ($inventoryType && $inventoryId) {
                // Verify inventory exists and reduce quantity
                DB::beginTransaction();
                try {
                    if ($inventoryType === 'egg') {
                        $eggInv = EggInventory::where('farm_owner_id', $farm_owner->id)
                            ->where('id', $inventoryId)
                            ->lockForUpdate()
                            ->first();

                        if (!$eggInv || $eggInv->quantity_available < $validated['quantity_available']) {
                            throw new \Exception('Insufficient egg inventory available');
                        }

                        // Reduce inventory and mark as reserved for product
                        $eggInv->quantity_available -= $validated['quantity_available'];
                        $eggInv->save();
                        $eggInventoryId = $inventoryId;

                    } elseif ($inventoryType === 'livestock') {
                        $livestockInv = LivestockInventory::where('farm_owner_id', $farm_owner->id)
                            ->where('id', $inventoryId)
                            ->lockForUpdate()
                            ->first();

                        if (!$livestockInv || $livestockInv->quantity_available_for_sale < $validated['quantity_available']) {
                            throw new \Exception('Insufficient livestock inventory available');
                        }

                        // Reduce inventory and mark as reserved for product
                        $livestockInv->quantity_available_for_sale -= $validated['quantity_available'];
                        $livestockInv->save();
                        $livestockInventoryId = $inventoryId;
                    }
                } catch (\Exception $e) {
                    DB::rollBack();
        ...
        $product = Product::create([
            ...
            'livestock_inventory_id' => $livestockInventoryId,
            'auto_sync_inventory' => ($eggInventoryId || $livestockInventoryId),
        ]);

        Log::info('Product created with inventory link', [
        ...
        return redirect()->route('products.show', $product)->with('success', 'Product created successfully' . (($eggInventoryId || $livestockInventoryId) ? ' and linked to inventory' : ''));
    }
