...
class DepartmentController extends Controller
{
    ...
    public function hr()
    {
        ...
        $stats = Cache::remember("dept_hr_{$farmOwnerId}", 120, function () use ($farmOwnerId) {
            return [
                'total_employees' => Employee::where('farm_owner_id', $farmOwnerId)->count(),
                'active_employees' => Employee::where('farm_owner_id', $farmOwnerId)->where('status', 'active')->count(),
                'pending_hires'    => Employee::where('farm_owner_id', $farmOwnerId)->where('status', 'pending')->count(),
                'departments'      => Employee::where('farm_owner_id', $farmOwnerId)->distinct('department')->count('department'),
            ];
        });
        ...
        $recentEmployees = Employee::where('farm_owner_id', $farmOwnerId)
            ->orderByDesc('created_at')
            ->limit(8)
            ->get(['id', 'employee_id', 'first_name', 'last_name', 'department', 'position', 'employment_type', 'status', 'created_at']);

        return view('department.hr', compact('stats', 'recentEmployees'));
    }
}
