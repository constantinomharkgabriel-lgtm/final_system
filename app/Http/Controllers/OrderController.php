...
class OrderController extends Controller
{
    ...
    public function index()
    {
        ...
        $orders = Order::where('consumer_id', $user->id)
            ->with(['farmOwner.user', 'items.product', 'delivery'])
            ->latest('created_at')
            ->paginate(20);
    ...
    public function show(Order $order)
    {
        ...
        $order->load(['delivery', 'items.product', 'farmOwner.user']);
    ...
    public function place_order(Request $request)
    {
        ...
        try {
            // Get farm owner to check order limit from first product
            $firstProductFarmOwnerId = null;
            foreach ($cart as $item) {
                $product = Product::find($item['product_id']);
                if ($product) {
                    $firstProductFarmOwnerId = $product->farm_owner_id;
                    break;
                }
            }

            if ($firstProductFarmOwnerId) {
                $farmOwner = FarmOwner::find($firstProductFarmOwnerId);
                if ($farmOwner) {
                    ...
                    if ($activeSubscription && $activeSubscription->order_limit) {
                        ...
                        $ordersThisMonth = Order::where('farm_owner_id', $farmOwner->id)
                            ...
                            ->whereRaw("TO_CHAR(created_at, 'YYYY-MM') = ?", [$currentMonth])
                            ->count();

                        if ($ordersThisMonth >= $activeSubscription->order_limit) {
                            return redirect()->route('products.index')
                                ->with('error', "You've reached the maximum of {$activeSubscription->order_limit} orders for your " . ucfirst($activeSubscription->plan_type) . " plan for this month. Please wait until next month or upgrade your subscription.");
                        }
                    }
                }
            }
            ...
            DB::transaction(function () use ($user, $cart, $validated, &$createdOrder) {
                ...
                // Store delivery coordinates in session for later use when confirming order
                if ($validated['delivery_latitude'] && $validated['delivery_longitude']) {
                    session()->put("delivery_coords.{$order->id}", [
    ...
    public function confirm_order(Order $order, Request $request)
    {
        ...
        // Automatically create delivery record for confirmed order
        try {
            // Check if delivery already exists
            if (!$order->delivery) {
                $consumer = $order->consumer;
                $farmOwner = $order->farmOwner;

                // Create delivery record
                $deliveryCoords = session()->get("delivery_coords.{$order->id}", []);
                
                $delivery = \App\Models\Delivery::create([
                    'farm_owner_id' => $farmOwner->id,
                    'order_id' => $order->id,
                    'tracking_number' => 'TRK-' . strtoupper(uniqid()),
                    'recipient_name' => $consumer->name ?? 'Customer',
                    'recipient_phone' => $consumer->phone ?? $order->delivery_address ?? 'N/A',
                ...
                // Fire OrderCreated event to trigger notifications
                \App\Events\OrderCreated::dispatch($order);
            }
        } catch (\Exception $e) {
            \Illuminate\Support\Facades\Log::error('Failed to auto-create delivery: ' . $e->getMessage());
        }

        return redirect()->back()->with('success', 'Order confirmed! Delivery record created automatically.');
    }
    ...
    private function createOrderPayment(Order $order, User $consumer): ?array
    {
        ...
        if ($checkoutData && $checkoutUrl) {
            ...
            // Cache the checkout session ID for payment verification
            Cache::put(
        ...
        // Cache the payment link ID for payment verification
        Cache::put(
    ...
    /**
     * Verify marketplace order payment status via PayMongo.
     * Called by mobile app or web after success redirect to ensure payment is confirmed.
     */
    public function verifyPaymentStatus(Request $request, Order $order)
    {
        $user = Auth::user();
        
        // Verify user owns this order (for web) or is authenticated (for API)
        if ($user->role !== 'farm_owner' && (int) $order->consumer_id !== (int) $user->id) {
        ...
        // Try to verify via cached checkout session ID or payment link ID
        $checkoutSessionId = Cache::get("web_checkout_session_{$order->id}") 
            ?? Cache::get("marketplace_checkout_session_{$order->id}");
        $paymentLinkId = Cache::get("web_payment_link_{$order->id}") 
            ?? Cache::get("marketplace_payment_link_{$order->id}");

        // If order has a stored PayMongo ID, use that as fallback
        $paymongoId = $checkoutSessionId ?? $paymentLinkId ?? $order->paymongo_payment_id;
        ...
        if (str_starts_with($paymongoId, 'cs_')) {
            // Checkout session
            $sessionData = $this->paymongo->retrieveCheckoutSession($paymongoId);
            if ($sessionData) {
                ...
                if ($status === 'succeeded' || !empty($payments)) {
                    // Payment succeeded - update order if not already marked
                    if ($order->payment_status !== 'paid') {
                        ...
                        Log::info('Order payment verified via success page', [
                            'order_id' => $order->id,
        ...
        } elseif (str_starts_with($paymongoId, 'link_')) {
            // Payment link
            $paymentData = $this->paymongo->retrievePaymentLink($paymongoId);
            if ($paymentData) {
                ...
                if ($status === 'paid') {
                    ...
                    if ($order->payment_status !== 'paid') {
                        ...
                        Log::info('Order payment verified via success page (payment link)', [
                            'order_id' => $order->id,
                            'payment_link_id' => $paymongoId,
                        ]);
                    }
