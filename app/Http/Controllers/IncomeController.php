...
class IncomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $stats = Cache::remember("farm_{$farmOwner->id}_income_stats", 300, function () use ($farmOwner) {
            ...
            return [
                'total_this_month' => IncomeRecord::byFarmOwner($farmOwner->id)
                    ...
                    ->where('income_date', '<', $nextMonthStart)
                    ->sum('amount'),
                'pending' => IncomeRecord::byFarmOwner($farmOwner->id)
                    ->where('payment_status', 'pending')
                    ->sum('amount'),
                'total_count' => IncomeRecord::byFarmOwner($farmOwner->id)->count(),
        ...
        // Use department view for department employees, farm owner view otherwise
        $view = Auth::user()->isFarmOwner() 
            ? 'farmowner.income.index' 
            : 'department.income';

        return view($view, compact('incomes', 'stats'));
    }
