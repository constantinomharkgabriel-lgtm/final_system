...
<body class="bg-gray-900 text-gray-100 min-h-screen">
    ...
    <script>
        // Auto-verify payment if returning from PayMongo
        const searchParams = new URLSearchParams(window.location.search);
        if (searchParams.has('payment') && searchParams.get('payment') === 'success' && '{{ $order->payment_status }}' === 'unpaid') {
            // Show loading indicator
            const notification = document.createElement('div');
            notification.className = 'fixed top-4 right-4 bg-blue-600 text-white px-4 py-3 rounded-lg z-50';
            notification.innerText = '⏳ Verifying payment with PayMongo...';
            document.body.appendChild(notification);

            // Call verification endpoint
            fetch('{{ route("orders.verify-payment", $order) }}', {
                method: 'GET',
                headers: {
            ...
            .then(data => {
                if (data.payment_status === 'paid') {
                    notification.className = 'fixed top-4 right-4 bg-green-600 text-white px-4 py-3 rounded-lg z-50';
                    notification.innerText = '✓ Payment verified! Refreshing...';
                    setTimeout(() => {
                        window.location.href = window.location.pathname + '?refresh=1';
                    }, 1500);
                } else {
                    notification.className = 'fixed top-4 right-4 bg-yellow-600 text-white px-4 py-3 rounded-lg z-50';
                    notification.innerText = '⏳ Payment verification in progress (check back in a moment)';
            ...
            .catch(error => {
                ...
                notification.innerText = 'Error verifying payment. Please try again.';
                setTimeout(() => notification.remove(), 5000);
            });
        }
    </script>
