...
<body class="bg-gray-900 text-gray-100 min-h-screen">
            ...
            <div class="rounded-xl border border-gray-700 bg-gray-800 p-4 shadow-sm">
                ...
                <form method="POST" action="{{ route('orders.place') }}" class="space-y-3">
                    ...
                    <div id="delivery_fields" class="space-y-3">
                        ...
                        <div>
                            <label class="block text-xs mb-1 text-slate-600">📍 Pin Your Delivery Location on Map</label>
                            <div id="delivery_map" style="height: 300px; border-radius: 0.5rem; border: 1px solid #cbd5e1;"></div>
                            <p class="text-xs text-gray-400 mt-1">Click on the map to set your delivery location</p>
    ...
    <script>
        // Initialize Leaflet map for delivery location picking
        let deliveryMap;
        let deliveryMarker;
        let geocodeTimeout;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Default center: Cavite, Philippines area
            const defaultLat = 14.3577;
            const defaultLng = 120.8854;

            // Initialize map centered on Cavite area  
            deliveryMap = L.map('delivery_map').setView([defaultLat, defaultLng], 15);
            ...
            const savedLat = document.getElementById('delivery_latitude').value;
            const savedLng = document.getElementById('delivery_longitude').value;

            if (savedLat && savedLng) {
                addDeliveryMarker(parseFloat(savedLat), parseFloat(savedLng));
            }
            ...
            // Auto-geocode address when user enters it
            ...
            function geocodeAddress() {
                ...
                if (!address || !city) return; // Need at least address and city
                ...
                fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(fullAddress)}&limit=1`)
                    ...
                    .then(data => {
                        if (data && data.length > 0) {
                            ...
                            // Verify location is in Cavite area (roughly)
            ...
            function addDeliveryMarker(lat, lng) {
                ...
                deliveryMarker = L.marker([lat, lng])
                    .addTo(deliveryMap)
                    .bindPopup(`<strong>Delivery Location</strong><br>Lat: ${lat.toFixed(6)}<br>Lng: ${lng.toFixed(6)}`)
                    .openPopup();

                document.getElementById('delivery_latitude').value = lat.toFixed(8);
                document.getElementById('delivery_longitude').value = lng.toFixed(8);
