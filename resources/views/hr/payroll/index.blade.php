...
@section('subheader', 'Prepare payroll records for farm-owner approval')
...
<div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
    ...
    <div class="bg-gray-800 border border-gray-700 rounded-lg p-4 border-l-4 border-yellow-600">
        <p class="text-gray-400 text-xs">Pending Finance</p>
    ...
    <div class="bg-gray-800 border border-gray-700 rounded-lg p-4 border-l-4 border-purple-600">
        ...
        <p class="text-2xl font-bold text-purple-600">{{ $stats['approved'] ?? 0 }}</p>
    </div>
    <div class="bg-gray-800 border border-gray-700 rounded-lg p-4 border-l-4 border-green-600">
        <p class="text-gray-400 text-xs">Paid</p>
...
<div class="bg-gray-800 border border-gray-700 rounded-lg p-4 mb-6">
    <form method="GET" class="flex flex-wrap gap-4">
        <select name="status" class="px-3 py-2 bg-gray-700 text-white border border-gray-600 rounded-lg focus:ring-2 focus:ring-orange-500">
        ...
        <input type="month" name="month" value="{{ request('month', now()->format('Y-m')) }}"
            class="px-3 py-2 bg-gray-700 text-white border border-gray-600 rounded-lg focus:ring-2 focus:ring-orange-500">
            ...
            <tbody class="divide-y divide-gray-600">
                ...
                <tr class="hover:bg-gray-700">
                    ...
                    <td class="px-6 py-4 space-x-2">
                        <a href="{{ route('payroll.show', $payroll) }}" class="text-blue-400 hover:text-blue-300 text-sm">View</a>
                        
                        @if(($payroll->workflow_status ?? 'draft') === 'pending_finance')
                        <form method="POST" action="{{ route('payroll.destroy', $payroll) }}" style="display:inline;" onsubmit="return confirm('Are you sure? This will delete the payroll and also affect Finance dashboard.');">
                            @csrf
                            @method('DELETE')
