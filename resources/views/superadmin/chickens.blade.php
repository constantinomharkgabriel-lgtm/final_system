    ...
    <div class="flex h-screen bg-[#1a202c] font-sans text-gray-200">
        <aside class="w-64 bg-[#111827] border-r border-gray-700 flex flex-col">
            ...
            <nav class="flex-1 px-4 space-y-1">
                ...
                <a href="{{ route('chickens.index') }}" class="flex items-center px-4 py-3 bg-[#ed8936] text-white rounded-lg font-semibold">
        ...
        <main class="flex-1 flex flex-col overflow-hidden">
            <header class="bg-[#111827] border-b border-gray-700 py-4 px-8 flex justify-between items-center">
                <h1 class="text-2xl font-semibold">Chicken Monitoring Records</h1>
            ...
            <div class="p-8 overflow-y-auto">
                <div class="bg-gradient-to-r from-[#111827] to-[#1f2937] border border-gray-700 rounded-2xl shadow-2xl overflow-hidden">
                    <div class="p-6 border-b border-gray-700">
                        <h3 class="text-[#ed8936] font-bold text-sm uppercase tracking-widest">All Records ({{ $chickenRecords->total() }})</h3>
                    </div>
                        ...
                        <table class="w-full">
                            <thead class="bg-gray-900/50 border-b border-gray-700">
                                <tr>
                                    ...
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-400 uppercase">Batch Name</th>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-400 uppercase">Current Count</th>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-400 uppercase">Mortality</th>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-400 uppercase">Health Status</th>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-400 uppercase">Feed (Bags)</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-700">
                                @forelse ($chickenRecords as $record)
                                    <tr class="hover:bg-gray-900/30 transition">
                                        <td class="px-6 py-4 text-sm">{{ $record->date_logged?->format('M d, Y') ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 text-sm text-gray-300">{{ $record->batch_name ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 text-sm">
                                            <span class="font-semibold text-[#4fd1c5]">{{ $record->current_count ?? 0 }}</span>
                                        </td>
                                        <td class="px-6 py-4 text-sm">
                                            <span class="text-red-400">{{ $record->mortality_count ?? 0 }}</span>
                                        </td>
                                        <td class="px-6 py-4 text-sm">
                                            <span class="px-3 py-1 rounded-full text-xs font-semibold
                                                @if($record->health_status === 'good')
                                                    bg-green-900/30 text-green-400
                                                @elseif($record->health_status === 'fair')
                                                    bg-yellow-900/30 text-yellow-400
                                                @else
                                                    bg-red-900/30 text-red-400
                                                @endif
                                            ">
                                                {{ ucfirst($record->health_status ?? 'fair') }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 text-sm">{{ $record->feed_consumed_bags ?? 0 }}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-6 py-8 text-center text-gray-500">
                                            No chicken monitoring records found.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
