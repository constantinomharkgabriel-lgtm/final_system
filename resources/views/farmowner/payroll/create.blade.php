...
@php
// Build employee data for JavaScript (daily_rate for each employee)
$employeeData = [];
foreach($employees as $employee) {
    $employeeData[$employee->id] = $employee->daily_rate ?? 0;
}
@endphp
...
<script>
// Employee daily rates mapping
const employeeRates = @json($employeeData);

// Auto-calculate Basic Pay when employee or days_worked changes
function calculateBasicPay() {
    const employeeId = document.querySelector('[name="employee_id"]').value;
    const daysWorked = parseFloat(document.querySelector('[name="days_worked"]').value) || 0;
    const basicPayField = document.querySelector('[name="basic_pay"]');
    
    if (employeeId && employeeRates[employeeId]) {
        const dailyRate = employeeRates[employeeId];
        const basicPay = dailyRate * daysWorked;
        basicPayField.value = basicPay.toFixed(2);
    }
}

// Trigger on employee selection
document.querySelector('[name="employee_id"]').addEventListener('change', calculateBasicPay);

// Trigger on days_worked change
document.querySelector('[name="days_worked"]').addEventListener('change', calculateBasicPay);
document.querySelector('[name="days_worked"]').addEventListener('input', calculateBasicPay);

// Calculate on page load if values were pre-filled
window.addEventListener('load', calculateBasicPay);
</script>
@endsection
