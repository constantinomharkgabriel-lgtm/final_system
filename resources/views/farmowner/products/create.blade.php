    ...
    <form action="{{ route('products.store') }}" method="POST" class="bg-gray-800 border border-gray-700 rounded-lg p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            <div id="minimum_order_wrap">
                ...
                <input type="number" id="minimum_order" name="minimum_order" value="{{ old('minimum_order', 1) }}" min="1"
                    class="w-full px-3 py-2 bg-gray-700 border border-gray-600 text-white placeholder-gray-400 rounded-lg focus:ring-2 focus:ring-green-500">
            ...
            <div>
                ...
                <select name="is_bulk_order_enabled" id="is_bulk_order_enabled"
                    class="w-full px-3 py-2 bg-gray-700 border border-gray-600 text-white rounded-lg focus:ring-2 focus:ring-green-500">
                    <option value="0" {{ old('is_bulk_order_enabled', '0') === '0' ? 'selected' : '' }}>Normal (any quantity)</option>
                    <option value="1" {{ old('is_bulk_order_enabled') === '1' ? 'selected' : '' }}>Bulk only (maramihan)</option>
                </select>
            ...
            <div id="order_quantity_step_wrap" class="md:col-span-2">
                ...
                <input type="number" name="order_quantity_step" id="order_quantity_step" value="{{ old('order_quantity_step', 1) }}" min="1"
                    class="w-full px-3 py-2 bg-gray-700 border border-gray-600 text-white placeholder-gray-400 rounded-lg focus:ring-2 focus:ring-green-500">
        ...
        <!-- Image -->
        <!-- Inventory Linking -->
        <h3 class="font-semibold text-lg mb-4 pb-2 border-b border-gray-600 text-white">Link to Inventory (Optional)</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            @if($preSelectedItem)
                <!-- Pre-selected from inventory page -->
                <div class="md:col-span-2 p-4 bg-green-900/30 border border-green-700 rounded">
                    <p class="text-sm text-green-300 mb-2">📦 Pre-selected from inventory:</p>
                    <p class="text-white font-medium">{{ $preSelectedItem['label'] }}</p>
                    <input type="hidden" name="inventory_type" value="{{ $preSelectedItem['type'] }}">
                    <input type="hidden" name="inventory_id" value="{{ $preSelectedItem['id'] }}">
                    <input type="hidden" name="link_to_inventory" value="1">
                </div>
            @elseif($availableInventory->count() > 0)
                <!-- Manual selection when not pre-selected -->
                <div class="md:col-span-2">
                    <label class="flex items-center mb-3">
                        <input type="checkbox" name="link_to_inventory" id="link_to_inventory" value="1"
                            class="w-4 h-4 text-green-600 bg-gray-700 border-gray-600 rounded">
                        <span class="ml-2 text-sm text-gray-300">Link this product to inventory (auto-reduce quantity when sold)</span>
                    </label>
                </div>
                <div id="inventory_selection_wrap" class="md:col-span-2 hidden">
                    <label class="block text-sm font-medium text-gray-300 mb-1">Select Inventory Item *</label>
            ...
            @else
                <div class="md:col-span-2 p-4 bg-yellow-900/30 border border-yellow-700 rounded">
                    <p class="text-sm text-yellow-300">No available inventory items to link. Create inventory items first.</p>
                </div>
            @endif
        </div>
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('products.index') }}" class="px-6 py-2 bg-gray-600 text-gray-200 rounded-lg hover:bg-gray-700">Cancel</a>
        </div>
    </form>
</div>

<script>
    const linkCheckbox = document.getElementById('link_to_inventory');
    const inventoryWrap = document.getElementById('inventory_selection_wrap');
    ...
    @foreach(old('order_quantity_options', []) as $choice)
