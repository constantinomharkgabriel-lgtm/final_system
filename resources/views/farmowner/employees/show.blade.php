...
<div class="mt-6 bg-gray-800 border border-gray-700 rounded-lg p-6">
    <h3 class="text-lg font-bold text-white mb-4">Roles & Permissions</h3>
    @php
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            @foreach($roles as $role)
                <div class="border border-gray-600 rounded-lg p-4 bg-gray-700/30">
                    <p class="text-white font-semibold">{{ $role->display_name }}</p>
                    <p class="text-gray-400 text-sm mt-1">{{ $roleDescriptions[$role->name] ?? $role->description }}</p>
                </div>
            @endforeach
        </div>
        @if($employee->driver)
        <div class="mt-4 p-4 border border-blue-600 rounded-lg bg-blue-900/20">
            <p class="text-blue-300 text-sm"><strong>Driver Profile:</strong> This employee has an active driver profile and earns delivery commission.</p>
        </div>
        @endif
    @else
